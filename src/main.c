...
#endif
uint32_t  h_ui_A_length                = 8192;
uint32_t  h_ui_B_length                = 8192;
uint32_t  h_ui_C_length                = 16384; //array to put values in
uint32_t  h_ui_Ct_length               = 16384; //for unsorted and sorted
uint32_t  RUNS                         = 1;
uint32_t  entropy                      = 28;
...
float testMerge(
    ...
    if (!verifyOutput((*C), (*CSorted), C_length, algoName, 0)) {
        //time = -1.0;
    }
    ...
    vec_t** CUnsorted, uint32_t runs)
{
    ...
    for (uint32_t run = 0; run < RUNS; run++) {
        ...
        if (avx512SortNoMergePathV2SerialTime >= 0.0) {
            avx512SortNoMergePathV2SerialTime += testSort<avx512SortNoMergePathV2<bitonicMergeReal>>(
                CUnsorted, C_length,
                CSorted, Ct_length,
    ...
    if (OutToFile) {
        ...
        writeToSortOut("AVX-512 Hybrid Merge Sort", entropy, C_length, avx512SortNoMergePathV2SerialTime);
        #endif
    }
