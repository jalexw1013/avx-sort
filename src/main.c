...
#endif
uint32_t  h_ui_A_length                = 32768;//8388608;
...
float testParallelMerge(
    ...
    //verify output is valid
    if (!verifyOutput((*C), (*CSorted), C_length, algoName, numberOfThreads)) {
        //time = -1.0;
    }
...
float testSort(
    ...
    // //verify output is valid
    // if (!verifyOutput((*CUnsorted), (*CSorted), C_length, algoName, 0)) {
    //     //time = -1.0;
    // }
    ...
    vec_t** CUnsorted, uint32_t runs)
{
    ...
    for (uint32_t run = 0; run < RUNS; run++) {
        #ifdef AVX512
        avx512ParallelMergeTime += testParallelMerge<avx512ParallelMerge>(
            A, A_length, B, B_length,
            C, Ct_length, CSorted,
            runs, "AVX-512 Parallel Merge");
        #endif
    ...
    if (OutToFile) {
        #ifdef AVX512
        writeToParallelMergeOut("AVX-512 Merge Path Based Parallel Merge", entropy, A_length, B_length, numberOfThreads,avx512ParallelMergeTime);
        #endif
    } else {
    ...
    vec_t** CUnsorted, uint32_t runs)
{
    ...
    for (uint32_t run = 0; run < RUNS; run++) {
        ...
        //
            ...
            // avx512SortNoMergePathV2Time += testSort<avx512SortNoMergePathV2<bitonicMergeReal>>(
            //     CUnsorted, C_length,
            //     CSorted, Ct_length,
    ...
    if (OutToFile) {
        ...
        // if (isPowerOfTwo(C_length)) {
        //     writeToSortOut("AVX-512 Hybrid Merge Sort", entropy, C_length, avx512SortNoMergePathV2Time);
        // }
        #endif
    ...
    vec_t** CUnsorted, uint32_t runs)
{
    ...
    for (uint32_t run = 0; run < RUNS; run++) {
        ...
        #ifdef AVX512
        //if (isPowerOfTwo(C_length)) {
            avx512MergeParallelSortNewTime += testParallelSort<parallelIterativeMergeSort<avx512SortNoMergePathV2<avx512Merge>,avx512Merge>>(
                                                CUnsorted, C_length,
                                                CSorted, Ct_length,
