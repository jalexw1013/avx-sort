    ...
    vec_t** array, uint32_t array_length, const uint32_t splitNumber)
{
        ...
        #pragma omp parallel
        {
            ...
            if (earlyEnd) {
                ...
                //allocate splitters
                uint32_t* ASplitters = (uint32_t*)xcalloc(omp_get_num_threads(), sizeof(vec_t));
                uint32_t* BSplitters = (uint32_t*)xcalloc(omp_get_num_threads(), sizeof(vec_t));
